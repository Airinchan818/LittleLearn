...
class Sequential :
    ...
    def __run_optimizer(self):
            ...
            if isinstance(self.__optimizer,optimizers.GradientDescent) :
                RuntimeError("This Optimizers Not suport for Neural Network ")
...
class AutoBuildModel : 
    ...
    def __init__(self,num_target = None ,type:Literal['text-sentiment-lstm','mlp-regression',
    ...
    def __build_model (self) : 
        ...
        if self.__type == 'text-sentiment-lstm' : 
            ...
            if self.__level == 'light' : 
                if self.class_target is None :
                    self.__model = Sequential([
                        layers.Embedding(input_dim=self.__max_vocab,output_dim=32),
                ...
                if self.class_target is None :
                    self.__model = Sequential([
                        layers.Embedding(input_dim=self.__max_vocab,output_dim=64),
                ...
                if self.class_target is None :
                    self.__model = Sequential([
                        layers.Embedding(self.__max_vocab,128),
                        layers.LSTM(64,return_sequence=True),
                        layers.LSTM(32,return_sequence=True),
                        layers.GlobalAveragePooling1D(),
                        layers.Dense(128,activation='relu'),
        ...
        if self.__type == 'text-sentiment-simple-rnn' :
            ...
            if self.__level == 'light' :
                if self.class_target is None :
                    self.__model = Sequential([
                        layers.Embedding(self.__max_vocab,16),
                        layers.SimpleRNN(32),
                        layers.GlobalAveragePooling1D(),
                        layers.Dense(32,activation='relu'),
                        layers.Dense(1,activation='sigmoid')
                    ...
                    self.__model = Sequential([
                        layers.Embedding(self.__max_vocab,32),
                        layers.SimpleRNN(64),
                        layers.GlobalAveragePooling1D(),
                        layers.Dense(32,activation='relu'),

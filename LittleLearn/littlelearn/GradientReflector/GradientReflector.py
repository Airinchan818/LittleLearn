...
class GradientReflector :
    ...
    def layernormalization_backend(self, gamma, beta, epsilon=1e-6):
        ...
        out = count_forward(x_mu=x_mu,gamma=gamma.tensor,beta= beta.tensor)
        output = GradientReflector(out, (self, gamma, beta), 'layer_normal_backend')
        def _backward():
            N = x.shape[-1]
            x_hat = x_mu / std 
            dy = output.gradient
            beta.gradient += np.sum(dy,axis=-1,keepdims=True)
            gamma.gradient += np.sum((dy * x_hat),axis=-1,keepdims=True)
    ...
    def gru_backend(self,hidden_state,weight=(),bias=(),return_sequence=False,return_state = False) : 
        ...
        for t in range(seq) : 
            ...
            hidden_prev.append(hidden_state.tensor)
        ...
        def backward() :
            ...
            for t in reversed(range(seq)) : 
                ...
                dx_t = d_comb_up [:,:dim] + d_comb_re[:,:dim] + d_comb_cand[:,:dim]
                dx.insert(0,dx_t)
    ...
    def kill_grad (self) :
        ...
        for n in topo :
            n.gradient = np.zeros_like(n.tensor)

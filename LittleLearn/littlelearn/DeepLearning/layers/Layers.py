...
class Dense :
    ...
    def update_weight(self,weight:list) :
        if not isinstance(weight,list) :
            raise RuntimeError("wrap layers weight be list")
        if isinstance(self.weight,ll.GradientReflector) :
            self.weight.tensor = weight[0]
            self.bias.tensor = weight[1]
        else :
...
class MultiHeadAttention :
    ...
    def __init__ (self,units,num_heads,output_dim = 0,use_casual_mask=False,return_attention_state=False,epsilon=1e-9,
                  initial_weight : Literal['normal','uniform'] = 'normal',
                  initial_bias : Literal ['zeros','random'] = 'zeros',
                  stop_jacobian=True,
                  derivative_func : Literal['Jacobian','Alternative']  = 'Alternative',
    ...
    def __call__ (self,Query,Keys,Values) :
        try :
            ...
            if self.use_Engine_grad is False :
                ...
                Query = ll.GradientReflector(Query)
...
class Embedding :
    ...
    def __init__ (self,input_dim,output_dim,initial_weight : Literal ['normal','uniform'] = 'normal') :
    ...
    def __build_weight(self) :
        try :
            ...
            if self.initial_weight not in ['normal','uniform'] :
                raise RuntimeError("initial weight just availabel for normal and uniform") 
            if self.initial_weight == 'normal' :
                self.weight = np.random.normal(loc=0,scale=0.1,size=(self.input_dim,self.output_dim))
            if self.initial_weight == 'uniform' :
                self.weight = np.random.uniform(low=-0.1,high=0.1,size=(self.input_dim,self.output_dim))
    ...
    def __call__ (self,x) :
        ...
        return x.embedding_back_end(self.weight)
...
class SimpleRNN :
    ...
    def __call__ (self,x) :
        ...
        return x.simple_rnn_backend(self.h,weight=(self.weight_sequence,self.weight_hidden),bias_= self.bias)
...
class LSTM :
    ...
    def __call__ (self,x,initial_state=None) : 
        ...
        return x.lstm_backend(hidden_state=self.hidden_state,
...
class GRU :
    ...
    def __call__(self,x,initial_state = None ) : 
        ...
        return x.gru_backend(self.hidden_state,

...
class Embedding :
    ...
    def __build_weight(self) :
        try :
            ...
            if self.initial_weight == 'normal' :
                self.weight = np.random.normal(loc=0,scale=1,size=(self.input_dim,self.output_dim))
            if self.initial_weight == 'uniform' :
                self.weight = np.random.uniform(low=-1,high=1,size=(self.input_dim,self.output_dim))
